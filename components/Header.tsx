        ...
        <div className="flex items-center gap-1 sm:gap-2 flex-1 justify-end ml-4">
           ...
           <div className="relative" ref={langRef}>
              <button 
                ...
                className={`w-9 h-9 flex items-center justify-center rounded-full transition-all ${isLangOpen ? 'bg-primary text-white shadow-lg' : 'hover:bg-white/5 text-slate-300'}`}
              ...
              {isLangOpen && (
                <div className="fixed top-[70px] left-4 right-4 sm:absolute sm:top-full sm:left-auto sm:right-0 mt-3 w-auto sm:w-64 bg-slate-900/95 backdrop-blur-xl rounded-[2rem] shadow-2xl border border-white/10 p-4 z-[100] animate-fade-in-up">
                  <p className="text-[10px] font-black uppercase tracking-widest text-primary mb-3 px-2">Select Language</p>
           ...
           <div className="relative" ref={promoRef}>
              ...
              {isPromoOpen && (
                <div className="fixed top-[70px] left-4 right-4 sm:absolute sm:top-full sm:left-auto sm:right-0 mt-3 w-auto sm:w-64 bg-slate-900 rounded-2xl shadow-2xl border border-white/10 p-4 z-[100] animate-fade-in-up">
           ...
           <div className="relative" ref={settingsRef}>
              <button 
                ...
                className={`w-9 h-9 flex items-center justify-center rounded-full transition-all ${isSettingsOpen ? 'bg-primary text-white shadow-lg' : 'hover:bg-white/5 text-slate-300'}`}
              >
                <span className="material-symbols-outlined text-[24px]">settings</span>
              </button>
              {isSettingsOpen && (
                <div className="fixed top-[70px] left-4 right-4 sm:absolute sm:top-full sm:left-auto sm:right-0 mt-3 w-auto sm:w-72 bg-slate-900 rounded-2xl shadow-2xl border border-white/10 p-2 flex flex-col gap-1 z-[100] animate-fade-in-up">
                     ...
                     <div className="flex flex-col gap-2">
                       ...
                       <button 
                         onClick={() => {
                           ...
                           setTimeout(() => { setShowKeySaved(false); setSettingsOpen(false); window.location.reload(); }, 1500);
                         }}
                   ...
                   <button 
                     onClick={() => { setSettingsOpen(false); onShowAbout(); }}
                   ...
                   {user ? (
                     ...
                     <button 
                       onClick={() => { setSettingsOpen(false); onLogin(); }}
           ...
           <div className="relative w-8 h-8 flex items-center justify-center shrink-0" ref={profileRef}>
             {user ? (
               <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="w-7 h-7 rounded-full overflow-hidden border border-primary/30 transition-transform active:scale-95 text-left">
             ...
             {isProfileOpen && user && (
               <div className="fixed top-[70px] right-4 left-auto sm:absolute sm:top-full sm:left-auto sm:translate-x-0 sm:right-0 mt-3 w-48 bg-slate-900/95 backdrop-blur-xl rounded-[2rem] shadow-2xl border border-white/10 p-2 flex flex-col gap-1 z-[110] animate-fade-in-up">
                 ...
                 >
                   <span className="text-[11px] font-black uppercase tracking-widest pl-2">Logout</span>
                   <span className="material-symbols-outlined text-[20px] group-hover:translate-x-1 transition-transform">logout</span>
                 </button>
               </div>
             )}
